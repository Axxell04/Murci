...
export async function getExpenses (page: number = 1, limit: number = 10) {
    ...
    const expenses = await getDb().select().from(table.expense).limit(limit).offset(offset).orderBy(table.expense.createdAt).execute()
...
export async function updateExpense (id: string, value?: number, reason?: string) {
    if (typeof value !== 'undefined' && typeof reason !== 'undefined') {
        await getDb().update(table.expense).set({value, reason}).where(eq(table.expense.id, id)).execute();
