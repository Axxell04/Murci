...
export async function getRevenues (page: number = 1, limit: number = 5) {
    ...
    const revenues = await getDb().select().from(table.revenue).limit(limit).offset(offset).orderBy(desc(table.revenue.createdAt)).execute()
...
export async function updateRevenue (id: string, value?: number, reason?: string) {
    const [order] = await getDb().select().from(table.order).where(eq(table.order.revenueId, id)).execute();
    if (order) { return }
    if (typeof value !== 'undefined' && typeof reason !== 'undefined') {
        await getDb().update(table.revenue).set({value, reason}).where(eq(table.revenue.id, id)).execute();
